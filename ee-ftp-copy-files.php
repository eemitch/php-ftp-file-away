<?php // EE FTP Copy Directory - copyright Mitchell Bennis (mitch@elementengage.com) - rev 05/09/16
...
function list_dir_contents($path) {
	...
	if ($contents) {
		...
		$errors[] = "Could not list contents of $path";
	}
}
...
function parse_dir_contents($contents) {
	...
	global $conn, $path, $ftpRemote, $errors, $messages, $localDir, $fileFilter, $fileMaxSize, $remoteSize, $counter, $listOnly;
	...
	if(is_array($contents)) {
		...
		foreach ($contents as $item) {
			...
			if(strpos($item, '.') !== 0) { // Not folders upwards
				...
				$remotePathToFile = $ftpRemote . '/' . $item;
				
				if(ftp_is_dir($remotePathToFile)) {
					
					$messages[] = 'Folder Found: ' . $item;
					
/*
					$path .= '/' . $item; // Path is now /$item
					
					$path = str_replace('//', '/', $path); // Fix double slashes 
					
					if(!is_dir($localDir . $path)) {
					
						if(!mkdir($localDir . $path)) {
							$errors[] = 'Cannot create ' . $localDir . $path;
							return FALSE;
						} else {
							
							$messages[] = "Created Directory: " . $localDir . $path;
						}
					
					} else {
						
						$messages[] = "Directory Already Exists: " . $localDir . $path;
					}
					 
					
					if(ftp_chdir($conn, $path)) { // Change to this dir
					
						$subdir = list_dir_contents($path);
						
						parse_dir_contents($subdir);
						
						ftp_cdup($conn); // Change to parent directory
						
						$path = ftp_pwd($conn); // Get new dir name
						
						// $messages[] = 'DIRECTORY: ' . $path;
						
					} else {
					
						$errors[] = 'Could not change to ' . $path;
					}
*/
					...
					if(is_readable($local)) {
						...
						// Compare file sizes
						if(filesize($local) != ftp_size($conn, $remote)) {
							$errors[] = 'WARNING: File Size Mismatch for ' . $file;
							$errors[] = 'Local = ' . filesize($local) . ' / Remote = ' . ftp_size($conn, $remote);
						}
						...
						if(!$filtered) {
							...
							if(!$listOnly) {
								...
								if(ftp_get($conn, $local, $remote, FTP_BINARY)) {
								    ...
								    $errors[] = 'There was a problem fetching ' . $remote;
								}
								
							} else {
								
								$messages[] = '(Fetching: ' . $remote . ')';
								$messages[] = '(Saving To: ' . $local . ')';
